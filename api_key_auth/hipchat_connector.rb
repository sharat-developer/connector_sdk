     ...
     triggers: {
      New_message: {
        ...
        webhook_subscribe: ->(webhook_url, connection, input, flow_id) {
          url = "https://#{connection['deployment']}.hipchat.com/v2/room/#{input['room']}/extension/webhook/1"
          post(url,
        ...
        poll: ->(connection, input, page) {
          ...
          page ||= 0
          index = max * page
          response = get("https://#{connection['deployment']}.hipchat.com/v2/room/#{input['room']}/history?start-index=#{index}&max-results=#{max}")
					next_page = response['items'].length >= 100
          {
            events: response['items'],
            next_poll: (next_page ? page + 1 : nil),
            can_poll_more: next_page
          }
        },
