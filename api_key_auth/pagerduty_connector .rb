  ...
  actions: {
    get_incident_by_ID: {
      ...
      sample_output: ->(connection) {
        i = get("https://api.pagerduty.com/incidents")['incidents'].first['id']
        get("https://api.pagerduty.com/incidents/#{i}")
      }
    },
    ...
    update_incident: {
      ...
      input_fields: ->() {
        [
          ...
          {name: 'type',optional: false,control_type: 'select', pick_list: [["Incident","incident"],["Incident Reference","incdient_reference"]]},
          {name: 'status',control_type: 'select', pick_list: [["Acknowledged","acknowledged"],["Resolved","resolved"]]},
        ]
      },
      
      execute: ->(connection, input) {
         hash = {"incident" => {'type'=>input['type'],'status'=>input['status']},}
         put("https://api.pagerduty.com/incidents/#{input.delete('id')}", hash).headers(From: input['From'] )
      },
    ...
    search_incident: {
      ...
      input_fields: ->() {
        [
          ...
          {name: 'urgency',control_type: 'select', pick_list: [['high'], ['low']]},
          {name: 'time_zone'}
        ]
      },
  ...
  triggers: {
    new_incident: {
      ...
      poll: ->(connection,input,last_created_since) {
        ...
        response = get("https://api.pagerduty.com/incidents").params(since: since, sort_by: "created_at:DESC")
        last_created_since = response['incidents'].last['created_at'] unless response.blank?
        {
          events: response['incidents'],
          next_page: response['limit'] >= 100 ? response['offset'] : nil
        }
      },
      ...
      sample_output: ->(connection) {
        get("https://api.pagerduty.com/incidents")['incidents'].first || {}
      }
    },
  },
}
