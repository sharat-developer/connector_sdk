  ...
  actions: {
    create_account: {
      ...
      input_fields: ->(object_definitions) {
       object_definitions['accounts'].required('SALES_UNIT_ID','OWNER_ID','ORGANIZATION')
      },
    ...
    create_product: {
      ...
      input_fields: ->(object_definitions) {
       object_definitions['product'].required('NAME')
      },
      execute: ->(connection, input) {
    ...
    create_lead: {
      ...
      input_fields: ->(object_definitions) {
       object_definitions['lead'].required('OPPORTUNITY_NAME','OWNER_ID','SALES_UNIT_ID')
      },
      execute: ->(connection, input) {
    ...
    create_contact: {
      ...
      input_fields: ->(object_definitions) {
        object_definitions['contact'].required('SALES_UNIT_ID','OWNER_ID','SURNAME')
      },
      execute: ->(connection, input) {
    ...
    create_opportunity: {
      ...
      input_fields: ->(object_definitions) {
  object_definitions['opportunity'].required('SALES_UNIT_ID','OWNER_ID','OPPORTUNITY_NAME','STAGE','OPPORTUNITY_VALUE_FOREGIN','CLOSING_DATE','ACCOUNT_ID')
      },
      execute: ->(connection, input) {
  ...
  triggers: {
    ...
    new_contact: {
      ...
      poll: ->(connection, input, last_created_since) {
        since = (last_created_since || input['since'].to_time.utc.iso8601||Time.now)
           responses = get("https://#{connection['deployment']}.pipelinersales.com/rest_services/v1/#{connection['spaceId']}/Contacts?limit=100&offset=0&sort=-CREATED&filter=CREATED::#{since}::ge")
   ...
   new_opportunity: {
     ...
     input_fields: ->() {
      ...
      poll: ->(connection, input, last_created_since) {
        since = (last_created_since || input['since'].to_time.utc.iso8601|| Time.now)
           responses = get("https://#{connection['deployment']}.pipelinersales.com/rest_services/v1/#{connection['spaceId']}/Opportunities?limit=100&offset=0&sort=-CREATED&filter=CREATED::#{since}::ge")
           last_created_since = responses.last['CREATED'].to_time.utc.iso8601 unless responses.blank?
        responses = responses.select do |response|
            response['CREATED'] == response['MODIFIED']
           end
  ...
  updated_contact: {
    ...
    poll: ->(connection, input, last_modified_since) {
        since = (last_modified_since || input['since'].to_time.utc.iso8601 || Time.now)
           responses = get("https://#{connection['deployment']}.pipelinersales.com/rest_services/v1/#{connection['spaceId']}/Contacts?limit=100&offset=0&sort=-MODIFIED&filter=MODIFIED::#{since}::ge")
   ...
   updated_opportunity: {
     ...
     poll: ->(connection, input, last_modified_since) {
        since = (last_modified_since || input['since'].to_time.utc.iso8601|| Time.now)
           responses = get("https://#{connection['deployment']}.pipelinersales.com/rest_services/v1/#{connection['spaceId']}/Opportunities?limit=100&offset=0&sort=-MODIFIED&filter=MODIFIED::#{since}::ge")
        last_modified_since = responses.last['MODIFIED'].to_time.utc.iso8601 unless responses.blank?
           responses = responses.select do |response|
             response['CREATED'] < response['MODIFIED']
           end
